/*  =========================================================================
    application_network_selftest.c - run selftests
...
{
    test_item_t *item;
    printf ("Running application_network selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef APPLICATION_NETWORK_BUILD_DRAFT_API // selftest is still in draft
        else
            application_network_private_selftest (verbose, item->subtest);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("application_network_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running application_network test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef APPLICATION_NETWORK_BUILD_DRAFT_API // selftest is still in draft
        else
            application_network_private_selftest (verbose, test->subtest);
#endif // APPLICATION_NETWORK_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
