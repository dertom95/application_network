...
IF %1.==--help. (
    ...
    ECHO    --without-test_appnet_client  do not build test_appnet_client.exe
    ECHO    --without-test_appnet_all  do not build test_appnet_all.exe
    ECHO    --without-application_network_selftest  do not build application_network_selftest.exe
    GOTO :eof
)
ECHO Configuring application_network...
...
ECHO. >> platform.h
ECHO #define APPLICATION_NETWORK_HAVE_WINDOWS 1>> platform.h
...
IF EXIST "..\..\..\libzmq" (
    ...
    ECHO application_network cannot build without libzmq
...
IF EXIST "..\..\..\czmq" (
    ...
    ECHO application_network cannot build without czmq
    ECHO Please clone https://github.com/zeromq/czmq.git, and then configure ^& build
    ECHO TODO: resolve this problem automatically.
    GOTO error
)
IF EXIST "..\..\..\zyre" (
    ECHO Building with zyre
    ECHO #define HAVE_ZYRE 1>> platform.h
) ELSE (
    ECHO Building without zyre
    ECHO application_network cannot build without zyre
    ECHO Please clone https://github.com/zeromq/zyre.git, and then configure ^& build
    ECHO TODO: resolve this problem automatically.
    GOTO error
)
IF EXIST "..\..\..\cjson" (
    ECHO Building with cjson
    ECHO #define HAVE_CJSON 1>> platform.h
) ELSE (
    ECHO Building without cjson
    ECHO application_network cannot build without cjson
    ECHO Please clone https://github.com/davegamble/cjson, and then configure ^& build
    ECHO TODO: resolve this problem automatically.
    GOTO error
)
...
:with_draft
    ...
    ECHO #define APPLICATION_NETWORK_BUILD_DRAFT_API 1>>platform.h
    GOTO end_draft
:no_draft
    ECHO Building without draft API (stable + legacy API)
    ECHO #undef APPLICATION_NETWORK_BUILD_DRAFT_API 1>>platform.h
:end_draft
ECHO. >> platform.h
